...
{                           /* a "for(i=0;i<cadlayers; ++i)" loop */
  ...
  if(layer==0) {
    ...
    if(RECT_OUTSIDE(x1,y1,x2,y2,xctx->areax1,xctx->areay1,xctx->areax2,xctx->areay2)) {
      ...
      svg_filledrect(SYMLAYER, xctx->inst[n].xx1, xctx->inst[n].yy1, xctx->inst[n].xx2, xctx->inst[n].yy2,
      0.0, 0, 0, -1, -1);
      xctx->inst[n].flags|=1;
      return;
    }
  ...
  if( (!hide && xctx->enable_layer[layer]) ||
    ...
    for(j=0;j< symptr->rects[layer]; ++j)
    {
      ...
      if(layer == GRIDLAYER && rect->flags & 1024) {
        ...
        svg_filledrect(c, x0+x1, y0+y1, x0+x2, y0+y2, rect->bus, dash, rect->fill, ellipse_a, ellipse_b);
      }
    }
  }
